...
- Согласование по почте (включается IMAP_HOST в .env, для mail.ru
  imap.mail.ru): HTML-письмо с mailto-кнопками формирует ответ на служебный
  адрес, IMAP-поллер парсит тему `[BPM] APPROVE|REJECT <jwt>` изнутри сети.
  Токен подписан, привязан к задаче+пользователю; отправитель должен
  совпасть с email пользователя. Работает и без корп-сети (телефон)
